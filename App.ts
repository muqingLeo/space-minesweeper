...
export class App extends gfx.GfxApp
{
    ...
    constructor()
    {
        ...
        this.starfield = new gfx.Particles2(this.star, 300); //modified 200->300 to get more densed field
    ...
    update(deltaTime: number): void 
    {
        ...
        const mineSpeed = 0.025 * deltaTime;
        ...
        for(let i=0; i < this.starfield.numParticles; i++)
        {
           ...
           // ADD YOUR CODE HERE
                const starSize = this.starfield.particleSizes[i];
                //const starSpeedMeter = Math.sqrt(starSize.x*starSize.x + starSize.y*starSize.y);
                const starSpeed = 0.0075 / starSize;

                const XPos = (-1) * moveDirection.x * starSpeed * shipSpeed;
                const YPos = (-1) * moveDirection.y * starSpeed * shipSpeed;
        ...
        this.mines.children.forEach((mine: gfx.Node2) => {
            ...
            // ADD YOUR CODE HERE
            const mineXPos = -12.5*moveDirection.x * mineSpeed;
            const mineYPos = -12.5*moveDirection.y * mineSpeed;   

            mine.position.x += mineXPos;
            mine.position.y += mineYPos;
        ...
        this.lasers.children.forEach((laser: gfx.Node2) => {
            ...
            // ADD YOUR CODE HERE
            const rotationRadians = laser.rotation
            const XPosition = 0.05 * (Math.sin(laser.rotation));
            const YPosition = 0.05 * (Math.cos(laser.rotation));
            const LaserMovingVec = new gfx.Vector2(XPosition, YPosition);
            const NormalizedDirection = gfx.Vector2.normalize(LaserMovingVec);
            
            const XtoMove = -0.40 * NormalizedDirection.x * laserSpeed;
            const YtoMove = 0.40 * NormalizedDirection.y * laserSpeed;
            laser.position.x += XtoMove;
            laser.position.y += YtoMove;

            if (laser.position.x > 1 || laser.position.y > 1 || laser.position.x < -1 || laser.position.y < -1) {
                laser.remove();
            }
    ...
    onMouseDown(event: MouseEvent): void 
    {
        ...
        const spawnLaser = gfx.Geometry2Factory.createBox();
        spawnLaser.position.set(this.ship.position.x, this.ship.position.y);
        spawnLaser.rotation = this.ship.rotation;
        // this.laser.position.set(this.ship.position.x, this.ship.position.y);
        // this.laser.rotation = this.ship.rotation;
        spawnLaser.material.color.set(230/255, 25/255, 230/255, 1);
        spawnLaser.scale = new gfx.Vector2(0.005, 0.05);
        // this.laser.material.color.set(25/255, 227/255, 49/255, 1);
        // this.laser.scale = new gfx.Vector2(0.005, 0.05);


        
        this.lasers.add(spawnLaser);
    ...
    //NEW: the ship will collide with a mine, mine will explode upon collision with the ship
    private checkForMineCollisions(): void
    {
        ...
        // traverse through mines to see if any collison will happen
        for (let i=0; i<this.mines.children.length; i++)
        {
            const mine = this.mines.children[i] as Mine;
            if (mine.intersects(this.ship, gfx.IntersectionMode2.BOUNDING_CIRCLE)){
                mine.explode();
    ...
    // NEW: explosion sound effect added when mines collide
        ...
        this.lasers.children.forEach((laser: gfx.Node2)=>{
            this.mines.children.forEach((mineElem: gfx.Node2)=>{
                ...
                if (mine.intersects(laser, gfx.IntersectionMode2.AXIS_ALIGNED_BOUNDING_BOX)){
                    laser.remove();
                    const Explosion_ = new Audio('./explosion.mp3');
                    Explosion_.play();
                    // playing sound effect for explosion
                    mine.explode();
                }
            });
        });
